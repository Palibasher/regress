...
if uploaded_file:
    ...
    try:
        ...
        # Сохраняем фактические значения C5TC
        st.session_state['actual_C5TC'] = df['C5TC'].copy()
...
if df_f is not None:
    ...
    iqr_multiplier = st.slider("Множитель IQR", 1.0, 6.0, 1.5, 0.1, key="iqr_slider")
    ...
    if st.button("Очистить датафрейм от выбросов (IQR)", key="iqr_clean"):
        ...
        if st.button("Оставить выбросы", key="keep"):
            st.session_state.df_filtered = df_f.copy()
            st.success(f"Выполнено!")
...
if selected_features:
    ...
    if missing_data.any():
        ...
        st.session_state.df_cleaned = df_selected
else:
...
model_choice = st.radio(
    "Выберите модель:",
    ["Линейная регрессия", "Случайный лес", "Градиентный бустинг", "XGBoost", "LightGBM", "CatBoost"],
    horizontal=True)
with st.expander("ℹ️ Описание моделей"):
    st.markdown("""
    ### 🤖 Обзор моделей

    - **Линейная регрессия (Linear Regression):**
        Простейшая модель, предполагает линейную зависимость между признаками и целевой переменной. Быстрая, легко интерпретируемая, но может плохо работать с нелинейными зависимостями.

    - **Случайный лес (Random Forest):**
        Ансамбль деревьев решений. Хорошо работает "из коробки", устойчив к переобучению, может обрабатывать как линейные, так и нелинейные зависимости. Медленнее обучается и менее интерпретируем, чем линейные модели.

    - **Градиентный бустинг (Gradient Boosting):**
        Постепенно улучшает модель, обучаясь на ошибках предыдущих. Хорошо справляется с сложными задачами, но может переобучиться при недостаточной регуляризации. Относительно медленный.

    - **XGBoost:**
        Улучшенный вариант градиентного бустинга. Очень популярен в соревнованиях по машинному обучению. Быстрый, эффективный, поддерживает регуляризацию, автоматическую обработку пропусков и важность признаков.

    - **LightGBM:**
        Быстрая реализация градиентного бустинга от Microsoft. Особенно хорошо работает с большими объемами данных. Использует специальные алгоритмы (Histogram-based), что делает обучение очень быстрым.

    - **CatBoost:**
        Разработка Яндекса. Отлично справляется с категориальными признаками без необходимости в их ручной обработке. Устойчив к переобучению, требует минимальной настройки. Обучение чуть медленнее, чем у LightGBM, но простота использования — большой плюс.
    """)
# Подготовка данных
...
if model_choice == "Линейная регрессия":
    ...
    st.markdown(f"""
        - **R²:** {r2:.3f}
        """,help=("""
        R² (коэффициент детерминации)\n — это мера того, насколько хорошо модель объясняет вариацию в данных. Значение R² близкое к 1 указывает на то, что модель хорошо описывает данные, а значение, близкое к 0 — что модель не объясняет данные.
        """
        ))
    ...
    st.markdown(f"""
        ...
        """,help=("""
        Нормированный R²\n ормированный R²** учитывает количество признаков в модели и наказывает за добавление лишних признаков. Это помогает избежать переобучения модели. Его значение всегда ниже или равно R².
        """
        ))
    ...
    st.markdown(f"""
        ...
        """,help=("""
        Стандартная ошибка\n это мера разброса ошибок предсказания модели. Чем меньше это значение, тем точнее предсказания модели.
        """
        ))
    ...
    st.markdown(f"""
        ...
        """, help="""
        SST (Total Sum of Squares) — это сумма квадратов отклонений всех фактических наблюдений от средней величины целевой переменной. 
        Это общее отклонение в данных.
        """)

    st.markdown(f"""
        - **SSR (Сумма квадратов регрессии):** {ssr:.2f}
        """, help="""
        SSR (Sum of Squares for Regression) — это сумма квадратов отклонений прогнозных значений от средней величины целевой переменной. 
        Чем больше эта сумма, тем лучше модель объясняет данные.
        """)

    st.markdown(f"""
        - **SSE (Сумма квадратов ошибок):** {sse:.2f}
        """, help="""
        SSE (Sum of Squared Errors) — это сумма квадратов отклонений реальных значений от предсказанных. 
        Меньшее значение SSE означает, что модель точнее предсказывает данные.
        """)

    st.markdown(f"""
        - **MSR (Среднеквадратичное отклонение регрессии):** {msr:.3f}
        """, help="""
        MSR — это среднеквадратичное отклонение для регрессии, то есть SSR, деленная на степени свободы модели (количество признаков). 
        Это показатель того, насколько хорошо модель объясняет данные в среднем.
        """)

    st.markdown(f"""
        - **MSE (Среднеквадратичное отклонение ошибки):** {mse:.3f}
        """, help="""
        MSE (Mean Squared Error) — это среднее значение квадрата отклонений предсказанных значений от фактических. 
        Меньшее значение MSE означает, что модель имеет меньшие ошибки в предсказаниях.
        """)

    st.markdown(f"""
        - **F-статистика:** {f_stat:.2f}
        """, help="""
        F-статистика — это тест, который оценивает значимость модели. 
        Если F-статистика высокая, это означает, что модель значительно лучше предсказывает, чем случайные данные. 
        Зависит от отношения MSR к MSE.
        """)
    ...
    # Вывод
    st.subheader("📈 Результаты модели XGBoost")
    st.markdown(f"""
    - **R²:** {r2:.3f}
    - **Стандартная ошибка (RMSE):** {std_error:.3f}
    - **Средняя абсолютная ошибка (MAE):** {mae:.3f}
    ...
    fig2, ax2 = plt.subplots(figsize=(8, 5))
    feature_importance_sorted.plot(kind='barh', ax=ax2)
    ax2.set_title("Важность признаков по версии XGBoost")
    st.pyplot(fig2)
    ...
    # Вывод
    st.subheader("📈 Результаты модели LightGBM")
    st.markdown(f"""
    - **R²:** {r2:.3f}
    - **Стандартная ошибка (RMSE):** {std_error:.3f}
    - **Средняя абсолютная ошибка (MAE):** {mae:.3f}
    ...
    fig2, ax2 = plt.subplots(figsize=(8, 5))
    feature_importance_sorted.plot(kind='barh', ax=ax2)
    ax2.set_title("Важность признаков по версии LightGBM")
    st.pyplot(fig2)
    ...
    # Вывод
    st.subheader("📈 Результаты модели CatBoost")
    st.markdown(f"""
    - **R²:** {r2:.3f}
    - **Стандартная ошибка (RMSE):** {std_error:.3f}
    - **Средняя абсолютная ошибка (MAE):** {mae:.3f}
    ...
    fig2, ax2 = plt.subplots(figsize=(8, 5))
    feature_importance_sorted.plot(kind='barh', ax=ax2)
    ax2.set_title("Важность признаков по версии CatBoost")
    st.pyplot(fig2)
...
with st.expander("📊 Density Plot of Real vs Predicted C5TC"):
    ...
    ax_kde.legend()
    st.pyplot(fig_kde)
st.subheader("🔍 Ввести значения признаков для прогноза")

with st.expander("📊 Сделать прогоз на основе выбранных признаков"):
    with st.form("predict_form"):
        input_data = {f: st.number_input(f"{f}", value=0.0, format="%.2f")
    ...
    if submit:
        ...
        if use_scaling and scaler:
            try:
                input_df[selected_features] = scaler.transform(input_df)
            except Exception as e:
        ...
        try:
            ...
            st.success(f"🔮 Прогноз модели: {pred:.2f}")
        except Exception as e:
            st.error(f"Ошибка при прогнозе: {e}")


if st.button("📈 Факт vs Предсказание"):
    # Получаем даты для тестовой выборкиf
    dates = st.session_state['df_raw'].loc[y_test.index, 'Месяц']

    # Собираем DataFrame для вывода и сортируем по дате
    df_plot = pd.DataFrame({
        'Дата': dates,
        'Фактические': y_test.values,
        'Предсказанные': y_pred
    }).sort_values('Дата')

    # Рисуем линейный график
    fig, ax = plt.subplots(figsize=(24, 10))
    ax.plot(df_plot['Дата'], df_plot['Фактические'], '-o', label='Фактические', alpha=0.8)
    ax.plot(df_plot['Дата'], df_plot['Предсказанные'], '-x', label='Предсказанные', alpha=0.8)

    # Красивое форматирование дат на оси X

    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=2))  # Расставляем метки по месяцам
    plt.xticks(rotation=45)
    plt.tight_layout()

    # Дополнительное улучшение: подписям месяцев можно добавить ярлыки
    ax.set_xlabel("Дата")
    ax.set_ylabel("C5TC")
    ax.set_title("Факт vs Прогноз по дням")
    ax.legend()
